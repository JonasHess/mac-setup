...
_mcp_install_uv_git() {
  ...
  if [ -d "$dest/.git" ]; then
    # The previous run's `uv sync` / `uv python pin` can modify tracked files
    # in this clone (uv.lock, .python-version). A plain `git pull` then aborts
    # on the dirty tree — especially under a global `pull.rebase=true`, which
    # turns the pull into a rebase that refuses to run with unstaged changes.
    # Discard that generated drift (it is regenerated by the uv steps below)
    # and treat a failed update as non-fatal: an existing checkout is enough to
    # build from, matching the "degrade gracefully, don't abort" convention.
    run git -C "$dest" checkout -- . || true
    run git -C "$dest" pull --ff-only \
      || warn "MCP '$name': could not update $dest — using the existing checkout."
  else
